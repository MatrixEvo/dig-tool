...
while true; do
  ...
  [[ -z ${a_record} ]] || ptr_a_record=$(echo "${a_record}" | while read -r ptr_a; do dig -x "${ptr_a}" @8.8.8.8 | grep "PTR" | grep -v ";" ; done)
  [[ -z ${mail_record} ]] || ptr_mail_record=$(echo "${mail_record}" | while read -r ptr_mail; do dig -x "${ptr_mail}" @8.8.8.8 | grep "PTR" | grep -v ";" ; done)
  [[ -z ${webmail_record} ]] || ptr_webmail_record=$(echo "${webmail_record}" | while read -r ptr_webmail; do dig -x "${ptr_webmail}" @8.8.8.8 | grep "PTR" | grep -v ";" ; done)
